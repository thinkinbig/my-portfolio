...
import { ERPPreview } from "@/components/previews/ERPPreview";

export default function ERPPlugins() {
  const { language } = useLanguage();
  const project = (getI18nText<Project[]>(language, 'projects.items')).find(p => p.id === 'erp-plugins');
                ...
                {project.challenges.map((challenge, index) => (
                  <div key={index} className="prose dark:prose-invert max-w-none">
                    <ReactMarkdown
                      components={{
                        h3: ({ children }) => (
                          <h3 className="text-xl font-semibold text-primary mb-4">{children}</h3>
                        ),
                        ul: ({ children }) => (
